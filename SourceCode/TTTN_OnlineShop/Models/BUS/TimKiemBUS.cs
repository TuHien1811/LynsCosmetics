...
namespace TTTN_OnlineShop.Models.BUS
{
    public class TimKiemBUS
    {
        ...
        public static IEnumerable<Table_SanPham> TimKiem(string timKiem)
        {
            ...
            return db.Query<Table_SanPham>("select MaSanPham, Table_SanPham.MaDanhMuc, Table_SanPham.MaThuongHieu, " +
                "TenDanhMuc, TenThuongHieu, TenSanPham, GiaBan, MoTa, SoLuongDaBan, LuotView, TinhTrang, HinhMinhHoa, " +
                "HinhNho1, HinhNho2, HinhNho3,GhiChu, GioiThieu from Table_SanPham " +
                "inner join Table_ThuongHieu on Table_SanPham.MaThuongHieu = Table_ThuongHieu.MaThuongHieu " +
                "inner join Table_DanhMuc on Table_SanPham.MaDanhMuc = Table_DanhMuc.MaDanhMuc " +
                "where TenSanPham like N'%" + timKiem + "%' or TenThuongHieu like N'%" + timKiem + "%' or TenDanhMuc like N'%" + timKiem + "%'");
        }
        //---------------------------Code Admin
